...
from plugins.maimaidx_music import total_list, get_cover_len4_id,get_cover_len5_id
...
class ChartInfo(object):
    def __init__(self, idNum:str, diff:int, tp:str, achievement:float, ra:int, comboId:int, scoreId:int,
        ...
        self.ra = computeRaPlus(ds,achievement)
...
class DrawBest(object):
    ...
    def __init__(self, sdBest:BestList, dxBest:BestList, userName:str):
        ...
        self.dxRating = 0
        for sd in sdBest:
            self.sdRating += computeRaPlus(sd.ds, sd.achievement)
        for dx in dxBest:
            self.dxRating += computeRaPlus(dx.ds, dx.achievement)
    ...
    def _drawBestList(self, img:Image.Image, sdBest:BestList, dxBest:BestList):
        ...
        for num in range(0, len(sdBest)):
            ...
            if not os.path.exists(pngPath):
                download(self.cover_dir + f'{get_cover_len4_id(chartInfo.idNum)}.png',f'https://www.diving-fish.com/covers/{get_cover_len5_id(chartInfo.idNum)}.png')
            if not os.path.exists(pngPath):        
            ...
            tempDraw.text((8, 44), f'Rating+ {chartInfo.ds} -> {computeRaPlus(chartInfo.ds, chartInfo.achievement)}', 'white', font)
        ...
        for num in range(0, len(dxBest)):
            ...
            if not os.path.exists(pngPath):
                download(self.cover_dir + f'{get_cover_len5_id(chartInfo.idNum)}.png',f'https://www.diving-fish.com/covers/{get_cover_len5_id(chartInfo.idNum)}.png')
            if not os.path.exists(pngPath):
                pngPath = self.cover_dir + '1000.png'
            try:
                temp = Image.open(pngPath).convert('RGB')
            ...
            if chartInfo.comboId:
                ...
                temp.paste(comboImg, (103, 27), comboImg.split()[3])
            font = ImageFont.truetype('src/static/adobe_simhei.otf', 12, encoding='utf-8')
            tempDraw.text((8, 44), f'Rating+ {chartInfo.ds} -> {chartInfo.ra}', 'white', font)
            font = ImageFont.truetype('src/static/adobe_simhei.otf', 18, encoding='utf-8')
            tempDraw.text((8, 60), f'#{num + 1}', 'white', font)
    ...
    def draw(self):
        ...
        playCountInfo = f'Rating+ {self.sdRating} + {self.dxRating} = {self.playerRating}'
        ...
        authorBoardDraw.text((31, 28), '   Rating+ By\nParrotNotFound', 'black', font2)
...
async def generate50new(payload: Dict) -> Tuple[Optional[Image.Image], bool]:
    ...
    async with aiohttp.request("POST", "https://www.diving-fish.com/api/maimaidxprober/query/player", json=payload) as resp:
        ...
        for i in range(len(dx)):
            ...
            dx[i]['ra'] = int(single)
            #print(single)
            dx[i]['ds'] = int(100*diff)/100.0
        cover_dir = 'src/static/mai/cover/'
        for c in sd:
            sd_best.push(ChartInfo.from_json(c))
            #download(cover_dir + f'{get_cover_len4_id(c["song_id"])}.png',f'https://www.diving-fish.com/covers/{get_cover_len5_id(c["song_id"])}.png')
        for c in dx:
